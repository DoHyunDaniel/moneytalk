...
/**
 * 채팅 메시지의 저장, 조회, 삭제(나가기) 기능을 담당하는 서비스 클래스입니다. 메시지의 soft delete 처리, 읽음 여부,
 * 전송자/수신자 설정 등 도메인 로직을 포함합니다.
 */
@Service
@Slf4j
...
public class ChatMessageService {
	...
	/**
	 * 채팅 메시지를 저장합니다. Redis 발행은 Controller에서 처리하므로, Service는 DB 저장만 담당합니다.
	 */
	@Transactional
	public void saveMessage(ChatMessageDto dto) {
		...
		ChatMessage message = ChatMessage.builder().chatRoom(chatRoom).sender(sender).receiver(receiver)
				.message(dto.getMessage()).type(dto.getType()).imageUrl(dto.getImageUrl()).isRead(false)
				.isDeletedBySender(false).isDeletedByReceiver(false).sentAt(LocalDateTime.now()).build();
